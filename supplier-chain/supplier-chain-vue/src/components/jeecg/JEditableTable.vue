    ...
    <a-row type="flex">
      ...
      <a-col>
        ...
        <div v-if="actionButton" class="action-button">
          <a-button v-show="addShow" type="primary" icon="plus" @click="handleClickAdd" :disabled="disabled">新增</a-button>
          <span class="gap"></span>
          <template v-if="selectedRowIds.length>0">
            <a-popconfirm
              :title="`确定要删除这 ${selectedRowIds.length} 项吗?`"
              @confirm="handleConfirmDelete">
              <a-button v-show="addShow" type="primary" icon="minus" :disabled="disabled">删除</a-button>
              <span class="gap"></span>
            </a-popconfirm>
                    ...
                    <template v-else-if="col.type === formTypes.popup">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <j-popup
                          ...
                          :init-query-param="col.initQueryParam"
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        if (this.hasStatisticsColumn) {
          if (this.statisticsColumns.hasOwnProperty(key)) {
            ...
            this.inputValues.forEach(item => {
              ...
              if (value && count !== '-') {
                try {
                  count = math.add(count,Number.parseFloat(value))
                } catch (e) {
                  count = '-'
                }
              }
            })
            this.statisticsColumns[key] = key == 'totalPrice'? count.toFixed(2) : count.toFixed(3);
      ...
      handleClickAdd() {
        // if(this.columns[1].key === "contractListNo" && !this.columns[0].initQueryParam) {
      ...
      popupCallback(value, others, id, row, column, index){
        let rowIdPrefix = id.split('_')[0]
        let sclValue = value.split(',');
        let objectSize = sclValue.length
        let key = []
        let values = []
        let times = 0
        if(index===0){
          times = objectSize - this.rows.length
        }else {
          times = objectSize - (this.rows.length - index)
        }
        if(times>0){
        ...
        for (let rowKey in others) {
          key.push(rowKey)
          let msg = others[rowKey].split(',')
          values.push(msg)
        }
        for (let i = 0; i < objectSize; i++) {
          let item = {}
          for (let j = 0; j < key.length; j++) {
            item[key[j]] = values[j][i]
          }
          this.DatePopupCallback(sclValue[i],item,rowIdPrefix+this.rows[index].id, this.rows[index],column, index++);
        }
      },
      /** popup回调 */
      DatePopupCallback(value, others, id, row, column, index) {
        // 存储输入的值
        ...
        if (others) {
          ...
          Object.keys(others).forEach(key => {
            ...
            if (key === column.key) {
              ...
              if(key === 'wtMode') {
                let wtMode = '';
                let prodClassCode = others['prodClassCode']
                //中板理计 热轧冷轧圆钢磅计
                switch (prodClassCode) {
                  case 'H':
                    wtMode = '1'
                        break;
                  case 'F':
                ...
                //建筑材理计
                if(prodClassCode === 'B' && others['orderLen']) {
                  wtMode = others['orderLen'] > 0 ? '1':'0'
                }
                setValueItem.values[key] = wtMode == ''? others[key] : wtMode;
              } else {
