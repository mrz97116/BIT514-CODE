<template>
  <a-card :bordered="false">
    ...
    <a-modal
      title="输入业务编码"
      :width="400"
      :visible="bizModal.visible"
      @ok="handleBizModalOk"
      @cancel="handleBizModalCancel"
      cancelText="关闭">
      <a-textarea placeholder="请输入业务标签" :rows="5" v-model="bizModal.formData"/>
    </a-modal>
...
<script>
  ...
  export default {
    ...
    methods: {
      handleTest(record) {
        this.bizModal.ruleCode = record.ruleCode;
        this.bizModal.visible = true;
      },
      handleBizModalOk(){
        
        let closeLoading = this.$message.loading('生成中...', 0)
        postAction(this.url.test, {
          ruleCode: this.bizModal.ruleCode,
          formData:this.bizModal.formData
        }).then(res => {
          if (res.success) {
            this.$info({
              title: '填值规则功能测试',
