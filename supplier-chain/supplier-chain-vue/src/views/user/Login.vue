  ...
  <div class="main">
    <a-form :form="form" class="user-layout-login" ref="formLogin" id="formLogin">
      <a-tabs
        ...
        <a-tab-pane key="tab1" tab="请输入账号密码登录">
          <a-form-item>
            <a-input
              size="large"
...
<!--      </div>-->
    ...
    <login-change-password-modal ref="loginChangePassword"></login-change-password-modal>
  </div>
</template>
...
<script>
  ...
  import LoginChangePasswordModal from './LoginChangePasswordModal.vue'
          ...
          that.form.validateFields([ 'username', 'password','inputCode', 'rememberMe' ], { force: true }, (err, values) => {
            if (!err) {
              ...
              that.Login(loginParams).then((res) => {
                // 在后端对密码强弱做出判断, 如果是生产环境就强制要求用户更改密码
                if ((res.result.isProductionEnvironment !== undefined && res.result.isWeakPassword !== undefined)
                    && (res.result.isProductionEnvironment && res.result.isWeakPassword)) {
                  // 打开一个强制用户修改密码的对话框
                  this.$refs.loginChangePassword.show(res.result.userInfo.username)
                  that.loginBtn = false;
                } else {
