...
public class SM2SignVerUtils {
	...
	public static String Sign2SM2(byte[] privatekey,byte[] sourceData) throws Exception{
		...
		//System.out.println("SM3摘要Z: " + HexUtil.getHexString(z));
		//System.out.println("被加密数据的16进制: " + HexUtil.getHexString(sourceData));
		sm2SignVO.setSm3_z(HexUtil.getHexString(z));
		sm2SignVO.setSign_express(HexUtil.getHexString(sourceData));

		sm3Digest.update(z, 0, z.length);
		sm3Digest.update(sourceData,0,sourceData.length);
		byte [] md = new byte[32];
		sm3Digest.doFinal(md, 0);
		//System.out.println("SM3摘要值: " + HexUtil.getHexString(md));
		sm2SignVO.setSm3_digest(HexUtil.getHexString(md));
		...
		String result = HexUtil.byteToHex(sign.getEncoded());
...
//		return sm2SignVO;
	...
	public static boolean VerifySignSM2(byte[] publicKey,byte[] sourceData,byte[] signData){
		try {
			...
			byte [] z = factory.sm2GetZ(USER_ID.getBytes(), userKey);
			//System.out.println("SM3摘要Z: " + HexUtil.getHexString(z));
			verifyVo.setSm3_z(HexUtil.getHexString(z));
			sm3Digest.update(z,0,z.length);
			sm3Digest.update(sourceData,0,sourceData.length);
			byte [] md = new byte[32];
			sm3Digest.doFinal(md, 0);
			//System.out.println("SM3摘要值: " + HexUtil.getHexString(md));
			verifyVo.setSm3_digest(HexUtil.getHexString(md));
	...
	public static void main(String[] args) throws Exception {
		...
		String sign = SM2SignVerUtils.Sign2SM2(HexUtil.hexStringToBytes(privatekey), sourceData);
		boolean verify = SM2SignVerUtils.VerifySignSM2(HexUtil.hexStringToBytes(publicKey), sourceData, HexUtil.hexStringToBytes(sign));
		System.out.println("签名值 "+sign);
		System.out.println("验签值:"+verify);
